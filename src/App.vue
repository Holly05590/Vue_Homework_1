...
<template>
  ...
  <div class="row">
    <!-- 商品列表區 -->
    <PorductList :products="products" @add-cart="addCart"></PorductList>

    <!-- 購物車區 -->
    <CartView :carts="carts" @remove-cart="removeCart"></cartView>
  </div>
...
<script setup>
  ...
  import PorductList from './views/PorductList.vue';
  ...
  const removeCart = (item) => {
    carts.value = carts.value.filter(cartItem => cartItem.id !== item.id);
  }
